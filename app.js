  // 1. Configurazione Firebase
    ...
    if (typeof firebase !== "undefined") {
        ...
        alert("Errore nel caricamento delle librerie Firebase. Controlla la connessione internet.");
    }

    // Costanti
    const INITIAL_ACCOUNT_BALANCE = 5348.43 ;
    const FIXED_INPS_INCOME = 101.90;
    ...
    // 3. Mappatura Anni e Mesi con dettagli base per la Modal Entrate
    const YEAR_MONTHS = {
        "2026": [
    ...
    // Calcola il saldo del Conto Generale considerando la base iniziale di Agosto già al netto e sommando i margini/INPS
// Assicurati che la costante sia impostata a 5884
...
function calculateCumulativeAccountBalance() {
    ...
    let balance = 5348.43; // Base iniziale esatta

    for (let i = 0; i <= targetIndex; i++) {
        const item = CHRONOLOGICAL_MONTHS[i];

        // 1. Entrate fisse del mese (1.901,90 € per Agosto/Settembre, 2.601,90 € da Ottobre)
        const monthConf = getCurrentMonthConfig();
        const yearMonths = YEAR_MONTHS[item.year] || [];
        const monthData = yearMonths.find(m => m.key === item.key) || { baseIncome: 1901.90 };
        const baseIncome = monthData.baseIncome;

        // 2. Entrate registrate (extra)
        const currentIncomes = Object.values(incomesData[item.key] || {});
        const extraIncome = currentIncomes.reduce((sum, entry) => sum + (parseFloat(entry.amount) || 0), 0);

        // 3. Uscite Fisse del mese
        const fixedExpenses = getMonthFixedExpenses(item.key);
        const totalFixed = fixedExpenses.reduce((sum, entry) => sum + (parseFloat(entry.amount) || 0), 0);

        // 4. Uscite Variabili (registrate)
        const currentExpenses = Object.values(expensesData[item.key] || {});
        const totalVariable = currentExpenses.reduce((sum, entry) => sum + (parseFloat(entry.amount) || 0), 0);

        // Formula: Saldo + (Entrate Fisse + Entrate Extra) - (Uscite Fisse + Uscite Variabili)
        const netMonth = (baseIncome + extraIncome) - (totalFixed + totalVariable);
        
        balance += netMonth;
    }

    const currentLabel = CHRONOLOGICAL_MONTHS[targetIndex] ? CHRONOLOGICAL_MONTHS[targetIndex].label : "";
    return { balance, currentLabel };
}
    ...
    function updateView() {
        ...
        document.getElementById("displayTotalAccount").innerText = formatEuro(accountInfo.balance);
    ...
    function openIncomeModal() {
        ...
        // Entrate Extra Registrate dal Form
        const currentIncomes = Object.values(incomesData[currentMonth] || {});
